...
def _extract_entry_tf(df: pd.DataFrame) -> Optional[str]:
    for column in ("entry_tf", "EntryTF", "entryTf", "ltfChoice", "ltf_choice", "LtfChoice"):
...
def _prepare_params(seed: Dict[str, object], entry_tf: Optional[str]) -> Dict[str, object]:
    ...
    if entry_tf:
        overrides["entry_tf"] = entry_tf
        overrides.setdefault("ltfChoice", entry_tf)
...
def _make_bank_payload(
    ...
    return {
        ...
        "metadata": {
            ...
            "htf_timeframe": metadata_source.get("htf_timeframe") or manifest.get("htf_timeframe"),
...
def run_retest(
    ...
    *,
    ...
    entry_tf = _extract_entry_tf(filtered)
    seed = _load_seed(selection.path / "seed.yaml")
    params_cfg = _prepare_params(seed, entry_tf)
