...
def equity_curve_from_returns(
    returns: pd.Series, initial: float = 1.0, *, max_clip: float = 5.0
) -> pd.Series:
    """로그 누적을 이용해 안정적인 자본曲선을 계산합니다."""
    ...
    equity = np.exp(log_equity) * float(initial)
    return pd.Series(equity, index=index, dtype=np.float64)
...
def finalise_metrics_result(
    ...
    *,
    ...
    total_assets = state.available_capital + state.savings + state.locked_capital
