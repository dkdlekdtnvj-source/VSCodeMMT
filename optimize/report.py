...
def _flatten_results(results: List[Dict[str, object]]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for record in results:
        ...
        params_payload = dict(record.get("params", {}) or {})
        timeframe_value = params_payload.get("timeframe")
        if timeframe_value is not None:
            base_row["timeframe"] = timeframe_value
        for key in (
            "timeframe",
            "timeframe_origin",
        ...
        base_row.update(params_payload)
        ...
        for dataset in record.get("datasets", []):
            ...
            for legacy_key in (
                ...
                params_payload.pop(legacy_key, None)
            ds_row.update(params_payload)
...
def export_results(
    ...
    *,
    ...
    agg_df = agg_df.drop(columns=drop_band_columns, errors="ignore")
    if not dataset_df.empty:
        dataset_df = dataset_df.drop(columns=drop_band_columns, errors="ignore")
...
def export_best(best: Dict[str, object], wf_summary: Dict[str, object], output_dir: Path) -> None:
    ...
    if isinstance(best.get("chart_timeframes"), list):
        payload["chart_timeframes"] = list(best["chart_timeframes"])
    if isinstance(best.get("entry_timeframes"), list):
        payload["entry_timeframes"] = list(best["entry_timeframes"])
...
def generate_reports(
    ...
    *,
    ...
    if isinstance(best_payload, dict):
        ...
        dataset_entries = best_payload.get("datasets") or []
        timeframe_set: OrderedDict[str, None] = OrderedDict()
        if isinstance(dataset_entries, list):
            for dataset in dataset_entries:
                ...
                if isinstance(meta, dict):
                    timeframe_value = str(meta.get("timeframe") or "").strip()
                    if timeframe_value and timeframe_value not in timeframe_set:
                        timeframe_set[timeframe_value] = None
        if timeframe_set and "timeframes" not in best_payload:
            best_payload["timeframes"] = list(timeframe_set.keys())
...
def write_trials_dataframe(
    ...
    *,
    ...
    def _param_series_value(key: str) -> pd.Series:
        if params_series is None:
            return pd.Series([None] * len(trials_df))
        return params_series.apply(
            lambda payload: payload.get(key) if isinstance(payload, dict) else None
        )

    timeframe_from_params = _param_series_value("timeframe")
    timeframe_from_meta = dataset_meta.apply(
        lambda meta: (
    ...
    timeframe_series = timeframe_from_params.where(
        timeframe_from_params.notna(), timeframe_from_meta
    )
    ...
    summary_columns: Dict[str, pd.Series] = {
        ...
        "BandExitMode": _attr_value("band_exit_mode"),
        "BandExitMult": _attr_value("band_exit_mult"),
        "BandExitSummary": _attr_value("band_exit_summary_text"),
