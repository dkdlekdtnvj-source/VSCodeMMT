...
def _normalize_post_sample_flags(
    ...
    has_dynamic = "useDynamicThresh" in patched
    has_static = "useSymThreshold" in patched

    if has_dynamic or has_static:
        dynamic_flag = (
            _coerce_bool_or_none(patched.get("useDynamicThresh")) if has_dynamic else None
        )
        static_flag = (
            _coerce_bool_or_none(patched.get("useSymThreshold")) if has_static else None
        )

        if has_dynamic and dynamic_flag is None:
            dynamic_flag = True

        if has_static and static_flag is None:
            # When both toggles exist we default to keeping the symmetric threshold off.
            static_flag = False if has_dynamic else True

        if has_dynamic and dynamic_flag is False:
            if has_static:
                if static_flag is False:
                    static_flag = True
            else:
                dynamic_flag = True

        if has_dynamic:
            patched["useDynamicThresh"] = dynamic_flag
            _update_trial_param(trial, "useDynamicThresh", dynamic_flag)

        if has_static:
            patched["useSymThreshold"] = static_flag
            _update_trial_param(trial, "useSymThreshold", static_flag)

    return patched


def _normalize_exit_flags(params: Dict[str, object]) -> Dict[str, object]:
    """
    Ensure mutually dependent exit toggles are well-behaved before exposing the
    sampled parameter set to the rest of the pipeline.

    When either ``exitOpposite`` or ``useMomFade`` is missing or resolved to
    ``None`` we provide sensible defaults and guarantee that at least one of the
    exits remains enabled.  This mirrors the post-sampling normalisation but
    avoids touching trial state during pure space sampling.
    """

    return _normalize_post_sample_flags(params, trial=None)
...
def sample_parameters(trial: optuna.Trial, space: SpaceSpec) -> Dict[str, object]:
    ...
    return _normalize_post_sample_flags(params, trial)
...
def random_parameters(space: SpaceSpec, rng: Optional[np.random.Generator] = None) -> Dict[str, object]:
    ...
    # We call the post-sample normaliser directly so that this helper remains
    # resilient even if `_normalize_exit_flags` is trimmed by tooling that
    # cherry-picks module globals.
    return _normalize_post_sample_flags(params, trial=None)
...
def mutate_around(
    ...
    mutated = _normalize_post_sample_flags(mutated)
    return mutated
