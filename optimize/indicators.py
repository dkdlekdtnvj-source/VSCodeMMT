...
def _compute_momentum_block(
    ...
    *,
    ...
    synthetic_high = df["high"].rolling(length, min_periods=1).max()
    synthetic_low = df["low"].rolling(length, min_periods=1).min()
    # Construct synthetic bars that mirror the Pine script's TR1 normalisation.
    ...
    tr1_raw = _true_range(synthetic_df)
    tr1_safe = tr1_raw.where((tr1_raw > 0) & tr1_raw.notna(), 1e-10)
    basis = _calc_basis(df, length, style)
    delta = df["close"] - basis
    
    # Pine Script: norm = tr1SafeVal != 0 ? delta / tr1SafeVal * 100.0 : 0.0
    delta_arr = delta.to_numpy(dtype=float, copy=False)
    tr1_arr = tr1_safe.to_numpy(dtype=float, copy=False)
    with np.errstate(divide="ignore", invalid="ignore"):
        output = np.zeros_like(delta_arr, dtype=float)
    ...
    norm = pd.Series(norm_values, index=df.index)
...
def _raw_directional_flux(df: pd.DataFrame, length: int) -> pd.Series:
    ...
    atr = _atr(df, length)
    atr_safe = atr.where((atr > 0) & atr.notna(), 1e-10)
    up = plus_r.divide(atr_safe)
    dn = minus_r.divide(atr_safe)
    denom = (up + dn).replace(0.0, np.nan)
    ratio = (up - dn).divide(denom).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    half_len = max(int(length // 2), 1)
    flux = _rma(ratio, half_len) * 100.0
