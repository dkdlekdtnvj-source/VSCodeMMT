...
PNL_FRACTION_LOWER_BOUND = -0.99
PNL_FRACTION_UPPER_BOUND = 10.0
_MIN_INITIAL_CAPITAL = 1e-9


def _normalise_pnl_fraction(pnl: float, initial_capital: float) -> float:
    """초기 자본 대비 수익률을 안전하게 정규화합니다."""

    base = max(initial_capital, _MIN_INITIAL_CAPITAL)
    fraction = pnl / base
    return float(np.clip(fraction, PNL_FRACTION_LOWER_BOUND, PNL_FRACTION_UPPER_BOUND))
...
# NEW: reason code for band exit (Bollinger or Keltner channel touch)
_FASTPATH_REASON_BAND = 3
...
if NUMBA_AVAILABLE:
    ...
    def _fastpath_calc_order_size(
        ...
        notional = qty * price
        cap_limit = tradable_capital * leverage * 1.05
        if not np.isfinite(notional) or notional > cap_limit:
            return 0.0
        return qty
    ...
    def _run_backtest_numba_fastpath(
        ...
        for idx in range(start_idx, n):
            ...
            if exit_triggered and position_dir != 0:
                ...
                if tradable_capital < min_equity_floor:
                    tradable_capital = min_equity_floor
                pnl_frac = _normalise_pnl_fraction(pnl, initial_capital)
                returns[idx] += pnl_frac
                ...
                position_entry_idx = -1
                bars_held = 0
                reentry_countdown = max(reentry_bars, 1)
                pyramid_count = 0
                base_entry_qty = 0.0
                continue
...
def run_backtest(
    ...
    def _apply_param_aliases(target: Dict[str, object]) -> Dict[str, object]:
        alias_map: Dict[str, Tuple[str, ...]] = {
            "basisStyle": ("basisStyleInput",),
            "maType": ("maTypeInput",),
            "leverage": ("leverageInput",),
            "entry_tf": ("entryTfSelection", "ltfChoice", "ltf_choices"),
            "useCapitalGuard": ("capitalGuard",),
            "useDailyLossGuard": ("dailyLossGuard",),
            "useDailyProfitLock": ("dailyProfitLock",),
            "useLossStreakGuard": ("lossStreakGuard",),
            "useWeeklyProfitLock": ("weeklyProfitLock",),
            "useBETiers": ("useBeTiers",),
            "useKASA": ("useKasa",),
            "kasa_rsiLen": ("kasaRsiLen",),
            "kasa_rsiOB": ("kasaRsiOB",),
            "kasa_rsiOS": ("kasaRsiOS",),
            "minHoldBars": ("minHoldBarsInput",),
        ...
        for canonical, aliases in alias_map.items():
            ...
            for alias in aliases:
                if alias in target and target[alias] is not None:
                    target[canonical] = target[alias]
                    break

        if "entry_tf" in target and isinstance(target["entry_tf"], str):
            target["entry_tf"] = target["entry_tf"].strip()

        if "fixedStopPct" not in target:
            long_val = target.get("fixedStopValueLong")
            short_val = target.get("fixedStopValueShort")
            extracted: List[float] = []
            for raw in (long_val, short_val):
                if raw is None:
                    continue
                try:
                    extracted.append(float(raw))
                except (TypeError, ValueError):
                    continue
            if extracted:
                chosen = extracted[0]
                if len(extracted) == 2 and not np.isclose(extracted[0], extracted[1], atol=1e-9):
                    LOGGER.warning(
                        "고정 손절 값(long=%s, short=%s)이 서로 다릅니다. 첫 번째 값 %.4f%%만 사용합니다.",
                        long_val,
                        short_val,
                        chosen,
                    )
                target["fixedStopPct"] = chosen

        return target

    params = _apply_param_aliases(params)

    if not params.get("startDate"):
        year = params.get("startYear")
        month = params.get("startMonth")
        day = params.get("startDay")
        try:
            if None not in (year, month, day):
                composed = datetime(int(year), int(month), int(day))
                params["startDate"] = composed.strftime("%Y-%m-%d")
        except Exception:
            LOGGER.debug("startYear/startMonth/startDay 조합으로 시작일을 구성하지 못했습니다.", exc_info=True)

    if "commissionPct" in params:
        try:
            commission_pct = float(params["commissionPct"])
        except (TypeError, ValueError):
            commission_pct = None
        if commission_pct is not None and np.isfinite(commission_pct):
            commission_decimal = commission_pct / 100.0
            fees.setdefault("commission_pct", commission_decimal)
            fees.setdefault("fee_pct", commission_decimal)

    if "commission_pct" not in fees and "fee_pct" in fees:
        fees["commission_pct"] = fees["fee_pct"]
    elif "fee_pct" not in fees and "commission_pct" in fees:
        fees["fee_pct"] = fees["commission_pct"]
    ...
    if chart_df is not None:
        chart_df = _ensure_datetime_index(chart_df, "차트")
        chart_df = _normalise_ohlcv(chart_df, "차트")
    else:
        chart_df = df
    ...
    # Pine ``requireCross`` 입력을 그대로 반영해 모멘텀 교차 필터 토글을 지원한다.
    require_momentum_cross = bool_param("requireCross", True)
    ...
    salvage_pct = 0.10
    # profit_deposit_pct: 수익의 몇 퍼센트를 저축/월렛에 적립할지 (useWallet 비활성화 시 0)
    profit_deposit_pct = 0.0
    # RUIN 임계값. 전체 자산(available_capital + savings)이 이 값 미만이면 백테스트를 중단합니다.
    ruin_threshold = 50.0
    drawdown_ruin_pct = 80.0
    ...
    if use_wallet:
        profit_deposit_pct = profit_reserve_pct
    apply_reserve_to_sizing = bool_param("applyReserveToSizing", True, enabled=use_wallet)
    min_tradable_capital = float_param("minTradableCapital", 150.0)
    ...
    max_drawdown_default = 70.0
    max_drawdown_limit = _resolve_requirement_value(
        "max_drawdown_pct",
        "maxDrawdownPct",
        "max_dd_pct",
        "maxDdPct",
        "maxDDPct",
        default=max_drawdown_default,
    )
    try:
        max_drawdown_limit_float = float(abs(max_drawdown_limit))
    except (TypeError, ValueError, OverflowError):
        max_drawdown_limit_float = max_drawdown_default
    if not np.isfinite(max_drawdown_limit_float):
        max_drawdown_limit_float = max_drawdown_default
    if max_drawdown_limit_float <= 0.0:
        max_drawdown_limit_float = float("inf")
    ...
    validity_thresholds = {
        ...
        "max_drawdown_pct": float(max_drawdown_limit_float),
    }
    ...
    use_stop_loss_param = bool_param("useStopLoss", False)
    use_fixed_stop = bool_param("useFixedStop", False)
    if fixed_stop_pct_val > 0:
        use_fixed_stop = True
    stop_lookback = int_param("stopLookback", 5)
    use_stop_loss = (
        use_stop_loss_param
        or use_fixed_stop
        or use_channel_stop
    ...
    returns_clip_upper = float_param("returnsClipUpper", 10.0)
    raw_max_equity_multiple = float_param("maxEquityMultiple", ANOMALY_EQUITY_MULT)
    if not np.isfinite(raw_max_equity_multiple):
        max_equity_multiple = ANOMALY_EQUITY_MULT
    else:
        max_equity_multiple = float(np.clip(raw_max_equity_multiple, 1.0, ANOMALY_EQUITY_MULT))
        if max_equity_multiple < raw_max_equity_multiple:
            LOGGER.warning(
                "maxEquityMultiple %.3g이 안전 상한 %.3g으로 클리핑되었습니다.",
                raw_max_equity_multiple,
                ANOMALY_EQUITY_MULT,
            )
    prune_on_anomaly = bool_param("pruneOnAnomaly", True)

    # Exit when price touches Bollinger Bands or Keltner Channels.  When
    # enabled, the backtester will trigger a stop if the high (for longs)
    # or low (for shorts) crosses either the upper or lower band.  This
    # complements existing stop mechanisms and disables the Numba fastpath.
    use_band_exit = bool_param("useBandExit", True)
    band_exit_min_bars = int_param("bandExitMinBars", 1, enabled=use_band_exit)
    LOGGER.debug(
        "Band exit toggle: enabled=%s, min_bars=%d",
        use_band_exit,
        int(band_exit_min_bars if use_band_exit else 0),
    )
    ...
    def _post_process_metrics(metrics: Dict[str, float]) -> Dict[str, float]:
        """Clip extreme returns and invalidate obvious overflow artefacts."""

        result = metrics
        returns_obj = result.get("Returns")
        if isinstance(returns_obj, pd.Series):
            upper_raw = float(np.clip(returns_clip_upper, 0.0, PNL_FRACTION_UPPER_BOUND))
            upper_clip = upper_raw if np.isfinite(upper_raw) else PNL_FRACTION_UPPER_BOUND
            clipped_returns = returns_obj.clip(
                lower=PNL_FRACTION_LOWER_BOUND,
                upper=upper_clip,
            )
            if not clipped_returns.equals(returns_obj):
                result = result.copy()
                result["Returns"] = clipped_returns.astype(np.float64)
            elif returns_obj.dtype != np.float64:
                result = result.copy()
                result["Returns"] = returns_obj.astype(np.float64)

        if not prune_on_anomaly:
            return result

        anomaly_reason = ""
        final_returns = result.get("Returns")
        returns_array = (
            final_returns.to_numpy(dtype=np.float64, copy=False)
            if isinstance(final_returns, pd.Series)
            else None
        )
        ...
        final_equity_val = _as_float(result.get("FinalEquity"))
        net_profit_val = _as_float(result.get("NetProfitAbs", result.get("NetProfit")))

        if not np.isfinite(final_equity_val) or not np.isfinite(net_profit_val):
            anomaly_reason = "non_finite_equity"
        elif returns_array is not None and returns_array.size:
            if not np.all(np.isfinite(returns_array)):
                anomaly_reason = "non_finite_returns"

        if not anomaly_reason:
            initial_capital_val = _as_float(initial_capital)
            if initial_capital_val > 0 and np.isfinite(max_equity_multiple):
                limit = initial_capital_val * float(max_equity_multiple)
                if np.isfinite(limit) and abs(final_equity_val) > limit:
                    anomaly_reason = "equity_overflow"
            if not anomaly_reason:
                trades_val = _as_float(result.get("Trades"))
                win_rate_val = _as_float(result.get("WinRate"))
                total_assets_val = _as_float(result.get("TotalAssets", final_equity_val))
                trade_min = 5.0
                if (
                    np.isfinite(trades_val)
                    and trades_val >= trade_min
                    and np.isfinite(win_rate_val)
                    and win_rate_val >= 0.999
                ):
                    anomaly_cap = float(max_equity_multiple) if np.isfinite(max_equity_multiple) else ANOMALY_EQUITY_MULT
                    cap_limit = initial_capital_val * anomaly_cap if np.isfinite(initial_capital_val) else ANOMALY_EQUITY_MULT
                    spike_floor = initial_capital_val * 500.0 if np.isfinite(initial_capital_val) else 0.0
                    spike_threshold = max(spike_floor, cap_limit * 0.5)
                    if np.isfinite(total_assets_val) and total_assets_val > spike_threshold:
                        anomaly_reason = "win_rate_spike"
                if (
                    not anomaly_reason
                    and np.isfinite(trades_val)
                    and trades_val >= trade_min
                    and np.isfinite(win_rate_val)
                    and win_rate_val >= 0.9
                ):
                    anomaly_reason = "win_rate_high"

        if anomaly_reason:
            if isinstance(final_returns, pd.Series):
                sanitized_returns_index = final_returns.index
            else:
                sanitized_returns_index = df.index[:0]
            sanitized = result.copy()
            sanitized["Valid"] = False
            sanitized["AnomalyPruned"] = True
            sanitized["AnomalyReason"] = anomaly_reason
            sanitized["AnomalyOriginalEquity"] = final_equity_val
            sanitized["AnomalyOriginalNetProfit"] = net_profit_val
            sanitized["FinalEquity"] = _as_float(initial_capital)
            sanitized["NetProfitAbs"] = 0.0
            sanitized["NetProfit"] = 0.0
            sanitized["TotalAssets"] = _as_float(initial_capital)
            sanitized["Trades"] = 0.0
            sanitized["TradesList"] = []
            sanitized["Returns"] = pd.Series(
                0.0,
                index=sanitized_returns_index,
                dtype=np.float64,
            )
            return sanitized
    ...
    def _attempt_fastpath() -> Optional[Dict[str, float]]:
        ...
        feature_flags = {
            ...
            "sizing_override": use_sizing_override,
                        # Disable Numba fastpath when band exit is enabled.  Band exits rely on
            # Python‑level logic and cannot currently be expressed in the
            # vectorised/Numa path.
            "band_exit": use_band_exit,
        }
        ...
        # FastPath 엔진에서 계산된 returns_array 또한 초기 자본 대비 소수 비율을 의미한다.
        # (예: 0.5 == 50% 수익률, 2.0 == 200% 초과 수익률)
        ...
        for i in range(total_trades):
            ...
            if reason_code == _FASTPATH_REASON_BAND:
                reason_text = "band_exit"
            elif reason_code == _FASTPATH_REASON_OPPOSITE:
            ...
            trades.append(
                Trade(
                    ...
                    profit=pnl,
                    return_pct=_normalise_pnl_fraction(pnl, initial_capital),
                    mfe=np.nan,
                    mae=np.nan,
                    bars_held=bars,
                    reason=reason_text,
        ...
        state = EquityState(
            ...
            reserve_for_sizing=use_wallet and apply_reserve_to_sizing,
        )
        metrics = finalise_metrics_result(
            state,
            trades,
            returns_series,
        ...
        return _post_process_metrics(metrics)
    ...
    chart_high = chart_df["high"]
    chart_low = chart_df["low"]

    def _align_chart_series(series: pd.Series) -> pd.Series:
        if not isinstance(series, pd.Series):
            series = pd.Series(series, index=chart_df.index, dtype=float)
        aligned = series.reindex(df.index, method="ffill")
        if aligned.dtype != float:
            aligned = aligned.astype(float)
        return aligned

    tick_size = _estimate_tick(chart_close)
    slip_value = tick_size * slippage_ticks

    # ATR 계산: 오실레이터 길이와 KC 길이 기반으로 각각 계산합니다.
    atr_len_series = _align_chart_series(_atr(chart_df, osc_len))
    ...
    # -------------------------------------------------------------------------
    # Band exit precomputation
    #
    # To support exiting trades when price touches the Bollinger Band or
    # Keltner Channel boundaries, we precompute boolean arrays indicating
    # whether the high (for longs) or low (for shorts) has crossed these
    # levels.  These arrays are later referenced by index inside the
    # main backtest loop.  Note: band exits require Python‑level logic and
    # therefore disable the Numba fastpath via the feature_flags above.
    bb_upper_series = bb_basis_close + bb_dev
    bb_lower_series = bb_basis_close - bb_dev
    # A long position should exit if the bar's high exceeds either the BB or
    # KC upper band.
    band_long_exit_bb = (df["high"] >= bb_upper_series)
    band_long_exit_kc = (df["high"] >= kc_upper)
    # A short position should exit if the bar's low falls below either the BB or
    # KC lower band.
    band_short_exit_bb = (df["low"] <= bb_lower_series)
    band_short_exit_kc = (df["low"] <= kc_lower)
    band_long_exit_bb_vals = band_long_exit_bb.to_numpy(dtype=np.bool_)
    band_long_exit_kc_vals = band_long_exit_kc.to_numpy(dtype=np.bool_)
    band_short_exit_bb_vals = band_short_exit_bb.to_numpy(dtype=np.bool_)
    band_short_exit_kc_vals = band_short_exit_kc.to_numpy(dtype=np.bool_)
    ...
    if use_atr_trail or use_breakeven_stop or use_be_tiers or use_dyn_vol or use_atr_profit:
        atr_trail_series = _align_chart_series(_atr(chart_df, atr_trail_len))
    else:
        atr_trail_series = pd.Series(np.nan, index=df.index)
    pivot_low_series = (
    ...
    swing_low_series = (
        _align_chart_series(chart_low.rolling(stop_lookback).min()) if use_stop_loss else pd.Series(np.nan, index=df.index)
    )
    swing_high_series = (
        _align_chart_series(chart_high.rolling(stop_lookback).max()) if use_stop_loss else pd.Series(np.nan, index=df.index)
    )
    sar_series = (
        None
        if use_sar_exit
    ...
    if use_shock:
        atr_fast_chart = _atr(chart_df, atr_fast_len)
        atr_fast = _align_chart_series(atr_fast_chart)
        atr_slow = _align_chart_series(_sma(atr_fast_chart, atr_slow_len))
    ...
    def check_ruin_floor() -> None:
        nonlocal ruin_hit, guard_frozen
        if ruin_hit:
            return
        total_assets = state.total_assets()
        if total_assets < ruin_threshold:
            ruin_hit = True
            guard_frozen = True
            return
        if state.peak_equity > 0:
            dd_pct = (state.peak_equity - state.equity) / state.peak_equity * 100.0
            if dd_pct >= drawdown_ruin_pct:
                ruin_hit = True
                guard_frozen = True
        if state.locked_capital <= 0.0 and state.tradable_capital < ruin_threshold:
            ruin_hit = True
            guard_frozen = True

    def sync_equity(update_trace: bool = False) -> None:
        state.update_tradable_capital()
        if update_trace:
    ...
    def close_position(ts: pd.Timestamp, price: float, reason: str) -> None:
        nonlocal position, highest_since_entry, lowest_since_entry, pos_bars, guard_frozen, guard_fired_total, last_position_dir, ruin_hit
        ...
        state.apply_pnl(pnl)
        returns_series.loc[ts] += _normalise_pnl_fraction(pnl, state.initial_capital)
        trades.append(
            Trade(
                ...
                exit_price=exit_price,
                profit=pnl,
                return_pct=_normalise_pnl_fraction(pnl, state.initial_capital),
                mfe=np.nan,
                mae=np.nan,
        ...
        ruin_hit = state.handle_trade_settlement(
            ...
            drawdown_ruin_pct=drawdown_ruin_pct,
        ) or ruin_hit
        if reason == "ruin":
            guard_frozen = True
    ...
    for idx, ts in enumerate(df.index):
        ...
        if ruin_hit:
            if position.direction != 0:
                close_position(ts, close_price, "ruin")
            break
        ...
        if position.direction == 0:
            if enter_long:
                ...
                if guard_ok and qty > 0 and capital_used > 0:
                    ...
                    fixed_dist = (
                        (fixed_stop_pct_val / 100.0) * close_price
                        if use_fixed_stop and fixed_stop_pct_val > 0
                        else 0.0
                    )
        ...
        if position.direction > 0:
            if use_band_exit and position.bars_held >= band_exit_min_bars:
                if band_long_exit_bb_vals[idx]:
                    exit_long = True
                    exit_long_reason = exit_long_reason or "band_exit_bb"
                if band_long_exit_kc_vals[idx]:
                    exit_long = True
                    exit_long_reason = exit_long_reason or "band_exit_kc"
            ...
            if use_band_exit and position.bars_held >= band_exit_min_bars:
                if band_short_exit_bb_vals[idx]:
                    exit_short = True
                    exit_short_reason = exit_short_reason or "band_exit_bb"
                if band_short_exit_kc_vals[idx]:
                    exit_short = True
                    exit_short_reason = exit_short_reason or "band_exit_kc"
    ...
    if use_band_exit:
        metrics["BandExitEnabled"] = 1.0
        metrics["BandExitBBMult"] = float(bb_mult)
        metrics["BandExitKCMult"] = float(kc_mult)
    else:
        metrics["BandExitEnabled"] = 0.0
        metrics.setdefault("BandExitBBMult", 0.0)
        metrics.setdefault("BandExitKCMult", 0.0)
    return _post_process_metrics(metrics)
